...
import { ProjectNetworkV1_0_0 } from '@subql/common-concordium';
import {
  ApiService,
...
> {
  ...
  async init(): Promise<ConcordiumApiService> {
    try {
      ...
      const endpointToApiIndex: Record<string, ConcordiumApiConnection> = {};

      for await (const [i, endpoint] of endpoints.entries()) {
        const connection = await ConcordiumApiConnection.create(
          endpoint,
