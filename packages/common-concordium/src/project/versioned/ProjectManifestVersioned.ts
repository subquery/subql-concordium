...
const Concordium_SUPPORTED_VERSIONS = {
...
export class ConcordiumProjectManifestVersioned implements IConcordiumProjectManifest {
  private _impl: ProjectManifestImpls;

  constructor(projectManifest: VersionedProjectManifest) {
    const klass = Concordium_SUPPORTED_VERSIONS[projectManifest.specVersion as Versions];
    if (!klass) {
